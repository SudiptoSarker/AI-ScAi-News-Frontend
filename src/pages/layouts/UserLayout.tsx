...
import Sidebar from "./sidebars/vertical/UserSidebar";
...
const UserLayout:React.FC<Props> = ({children})=> {
  ...
  const [userAuthReq, setUserAuthReq] = useState<boolean>(false);
  ...
  useEffect(()=> {
    ...
    setUserAuthReq(localStorage.getItem('userid') == 'undefined');    
  ...
  if(userAuthReq){
    return(
      <div>
        <h1>Your account is not authorized for user request. Please Retry.</h1>
      </div>
    );
  }
  return (
